...
from kb.offline.config.settings import EMBEDDING_MODEL, milvus_db_path
...
class MilvusRetriever:
    ...
    def save_vectorstore(self, docs: list[str]): 
        ...
        # 计算embedding。一次性计算全部文本的 dense/sparse 向量表示。
        texts_embeddings = embedding_handler(raw_texts)

        # batch embedding 插入， 按固定批次切片写入，降低单次写入开销并提升稳定性。
    ...
    def retrieve_topk(self, query, topk=10):
        t1 = time.time()
        # 抽取query的embedding ，将用户查询编码为 dense/sparse 两种向量。
        ...
        # 关联mongo数据，准备承接回表后的 Document 列表。
        related_docs = []
